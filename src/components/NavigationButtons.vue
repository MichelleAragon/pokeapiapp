...
const emit = defineEmits(["showAllPokemons", "showFavoritePokemons"]);

const handleShowAllPokemons = () => {
  emit("showAllPokemons");
};

const handleShowFavoritePokemons = () => {
  emit("showFavoritePokemons");
};
</script>

<template>
  <div
    class="fixed bottom-0 left-0 right-0 flex justify-center p-4 bg-white shadow-md z-[12]"
  >
    <div
      class="bg-white sm:w-[570px] flex justify-between gap-x-5 w-full"
    >
      <button
        @click="handleShowAllPokemons"
        :class="[
          {
            'bg-primary': currentView === 'all',
            'bg-disabled': currentView !== 'all',
          },
          'w-full px-4 py-2 text-white rounded-[60px] h-[44px] text-base font-bold flex items-center justify-center space-x-2',
        ]" >
        <svg
        ...
        <span class="capitalize bg-transparent">All</span>
      </button>
      <button
        @click="handleShowFavoritePokemons"
        :class="[
          {
            'bg-primary': currentView === 'favorites',
            'bg-disabled': currentView !== 'favorites',
          },
          'w-full px-4 py-2 text-white rounded-[60px] h-[44px] text-base font-bold flex items-center justify-center space-x-2',
        ]"
      >
        ...
        <span class="capitalize bg-transparent">Favorites</span>
      </button>
    </div>
  </div>
</template>
